...
<head>
    ...
    <title>MathVenture - Level 9 Perkalian Warna Warni</title>
...
<body>
    ...
    <div class="container">
        ...
        <h1>MathVenture</h1>
        <div class="level-info">Level 9: Perkalian</div>
...
<script>
        ...
        const level9Questions = [
           {question: "Raka memiliki 4 buku. Setiap buku memiliki 5 gambar. Berapa total gambar yang dimiliki Raka?",answer: 20,options: [20, 30, 40, 50]},
           {question: "Sebuah toko menjual 9 paket stiker. Setiap paket berisi 12 stiker. Berapa banyak stiker semuanya?",answer: 108,options: [109, 108, 110, 122]},
           {question: "Sebuah gudang memiliki 15 karung beras, tiap karung berisi 18 kg. Hari berikutnya datang tambahan 30 kg beras, lalu 95 kg dikirim ke warung. Berapa kg beras yang masih ada di gudang?",answer: 205,options: [210, 205, 215, 220]}
        ];
        // 📍 KONFIGURASI
        const MAP_URL = 'http://localhost/mathventure/pagepeta.php'; 
        const USER_ID = <?php echo json_encode($user_id); ?>; 
        const CURRENT_LEVEL_ID = 9;
        ...
        function levelComplete() {
            questionText.textContent = "Level 9 Selesai! 🎉";
		...
		// 🔓 Membuka Level Selanjutnya (Level 2)
		function unlockNextLevel() {
			const formData = new URLSearchParams();
			formData.append('user_id', USER_ID);
			formData.append('level_id', 10);
			...
			.then(r => r.json())
			.then(d => console.log("Unlock Level 10:", d));
		}
        ...
        function displayQuestion() {
            if (currentQuestionIndex >= level9Questions.length) {
                levelComplete();
                return;
            }
            // ... (Logika display soal lainnya) ...
            const currentQuestion = level9Questions[currentQuestionIndex];
        ...
        function checkAnswer(selectedOption, correctAnswer, clickedButton) {
            ...
            const isLastQuestion = currentQuestionIndex === level9Questions.length - 1;
            if (!isLastQuestion) {
